/* ============================================================
   GRADE 6 � SHARED MODULE — SHARED JAVASCRIPT MODULE
   ============================================================
...
function isTeacherUnlocked() {
  return localStorage.getItem('g6-teacher-unlock') === 'true'
    && !!localStorage.getItem('g6-teacher-name');
}

function toggleTeacherUnlock() {
  if (isTeacherUnlocked()) {
    if (confirm('Deactivate teacher unlock mode?')) {
      localStorage.removeItem('g6-teacher-unlock');
      removeUnlockBadge();
  ...
  if (code.trim() === 'shammel2026') {
    localStorage.setItem('g6-teacher-unlock', 'true');
    applyTeacherUnlock();
...
function googleSignIn() {
  ...
  localStorage.removeItem('g6-teacher-unlock');
  localStorage.removeItem('g6-teacher-name');
  localStorage.removeItem('g6-teacher-email');
  localStorage.removeItem('g6-teacher-avatar');
  localStorage.setItem('g6-student-name', name.trim());
  localStorage.setItem('g6-student-email', email.trim());
  checkSignIn();
}
function checkSignIn() {
  const name = localStorage.getItem('g6-student-name');
  const email = localStorage.getItem('g6-student-email');
...
function studentSignOut() {
  ...
  localStorage.removeItem('g6-student-name');
  localStorage.removeItem('g6-student-email');
  localStorage.removeItem('g6-teacher-unlock');
  localStorage.removeItem('g6-teacher-name');
  localStorage.removeItem('g6-teacher-email');
  localStorage.removeItem('g6-teacher-avatar');
...
function recordGrade(quizId, lessonTitle, score, total, passed) {
  const name = localStorage.getItem('g6-student-name') || 'Unknown';
  const email = localStorage.getItem('g6-student-email') || 'unknown';
  ...
  let grades = JSON.parse(localStorage.getItem('g6-grades') || '[]');
  ...
  localStorage.setItem('g6-grades', JSON.stringify(grades));
  const url = localStorage.getItem('g6-apps-script-url') || APPS_SCRIPT_URL;
  if (url) {
...
// ===== CSV EXPORT =====
function exportCSV() {
  const grades = JSON.parse(localStorage.getItem('g6-grades') || '[]');
  ...
  a.download = 'G6Platform_Grades_' + new Date().toISOString().slice(0, 10) + '.csv';
  a.click();
}
...
function exportJSON() {
  ...
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'G6Platform_Grades_' + new Date().toISOString().slice(0, 10) + '.json';
  a.click();
}
...
function clearGrades() {
  ...
  localStorage.removeItem('g6-grades');
...
function renderTeacherDashboard() {
  ...
  if (urlInput) urlInput.value = localStorage.getItem('g6-apps-script-url') || '';
}
...
function showResults(qid) {
  ...
  if (pass) {
    localStorage.setItem(`g6-unit${unitKey}-lesson${ln}-passed`, 'true');
    localStorage.setItem(`g6-unit${unitKey}-lesson${ln}-pct`, pct);
    if (typeof updateLock === 'function') updateLock();
...
function saveActState() {
  ...
  localStorage.setItem(`g6-unit${unitKey}-actComplete`, JSON.stringify(actComplete));
}

/** Restore actComplete state from localStorage */
function restoreActState() {
  const unitKey = document.body.dataset.unitKey || document.body.dataset.unit || 'a';
  const saved = localStorage.getItem(`g6-unit${unitKey}-actComplete`);
  if (saved) {
    try {
...
function markActivityDone(lesson, actName, score, total) {
  ...
  localStorage.setItem(`g6-unit${unitKey}-l${lesson}-${actName}-score`, `${score}/${total}`);
...
function checkGate(lesson) {
  ...
  if (btn && (allDone || isTeacherUnlocked())) {
    ...
    const unitKey = document.body.dataset.unitKey || document.body.dataset.unit || 'a';
    if (allDone) localStorage.setItem(`g6-unit${unitKey}-l${lesson}-gate`, 'true');
    updateLock();
  }
}
...
function saveCR() {
  ...
  if (el) localStorage.setItem(`g6-unit${unitKey}-l1-cr`, el.value);
}
function saveReflection() {
  const unitKey = document.body.dataset.unitKey || 'a';
  const el = document.getElementById('reflectionText');
  if (el) localStorage.setItem(`g6-unit${unitKey}-l1-reflect`, el.value);
}
function loadSavedText() {
  const unitKey = document.body.dataset.unitKey || 'a';
  const cr = document.getElementById('crResponse');
  const rf = document.getElementById('reflectionText');
  if (cr) cr.value = localStorage.getItem(`g6-unit${unitKey}-l1-cr`) || '';
  if (rf) rf.value = localStorage.getItem(`g6-unit${unitKey}-l1-reflect`) || '';
}
...
const APPS_SCRIPT_URL = localStorage.getItem('g6-apps-script-url') || 'https://script.google.com/macros/s/AKfycbwhr5HmNc1_5RBukfLmNdR6kWn9z6czPd-2IXZY93pGvodyf4NnZcGIAxkYwrgnmWybXQ/exec';
